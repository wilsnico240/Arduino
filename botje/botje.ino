...
const int PIN_BUTTON_MODE = 9;
const int PIN_BUTTON_SET = 12;
const int PIN_BUTTON_UP = 10;
const int PIN_BUTTON_MUTE = 9;
const int PIN_BUTTON_DOWN = 12;
const int PIN_BUTTON_DOWN2 = 11;
const int PIN_BUTTON_BREAK = 10;
const int PIN_BUTTON_BREAK2 = SW;
const int PIN_BUTTON_RTC = 11;
...
const unsigned long postingInterval2 = 300L * 1000L;
unsigned long lastConnectionTime3 = 0;
const unsigned long postingInterval3 = 1800L * 1000L;
...
String messagef = "   Radio <|> Temperature/Humidite";
matrix.beginDraw();
...
clearscreen();
matrix.renderBitmap(frame, 8, 12);
String messagef = "   Date/heure Internet <|> Config heure manuel";
matrix.beginDraw();
matrix.stroke(0xFFFFFFFF);
matrix.textScrollSpeed(30);
matrix.textFont(Font_5x7);
...
if (joystick_pos3 > -250 && joystick_pos3 <= 250) {
const char text[] = "Sna";
matrix.textFont(Font_4x6);
matrix.beginText(0, 1, 0xFFFFFF);
matrix.println(text);
...
matrix.stroke(0xFFFFFFFF);
matrix.textScrollSpeed(30);
const char text1[] = "   Pas de connexion WiFi, cette element ne fonctionne pas sans connexion Internet !!!";
matrix.textFont(Font_4x6);
matrix.beginText(12, 1, 0xFFFFFF);
...
if (houre < 10) {
if (Minute < 10) {
snprintf(timestamp85, 50, " 0%d:0%d", houre, Minute);
}
else if (houre < 10 && Minute >= 10) {
snprintf(timestamp85, 50, " 0%d:%d", houre, Minute);
} 
else {
snprintf(timestamp85, 50, " %d:%d", houre, Minute);
}}
else {
if (Minute < 10) {
snprintf(timestamp85, 50, " %d:0%d", houre, Minute);
}
else {
snprintf(timestamp85, 50, " %d:%d", houre, Minute);
}}
...
genSqr_Angle();
generateCHSVlut();
float dataRead = readFloatFromEEPROM(deviceAddress, memoryAddress);
radio.setFrequency(dataRead);
radio.setMuted(true);
}
...
matrix.stroke(0xFFFFFFFF);
matrix.textScrollSpeed(30);
const char text1[] = "   La synchronisation est malheureusement pas possible !!!";
matrix.textFont(Font_4x6);
matrix.beginText(12, 1, 0xFFFFFF);
matrix.println(text1);
matrix.endText(SCROLL_LEFT);
...
joystick_pos2 = analogRead(A0) - 512;
if (joystick_pos2 > 0) {
if (digitalRead(PIN_BUTTON_BREAK2) == LOW) {
clearscreen();
delay(250);
matrix2.clear();
leftEye();
...
matrix.renderBitmap(frame, 8, 12);
delay(500); 
writeFloatToEEPROM(deviceAddress, memoryAddress, frequency);
delay(500);
return;
}}
if (digitalRead(PIN_BUTTON_MUTE) == LOW) {
radio.setMuted(true);
clearscreen();
delay(250);
matrix2.clear();
...
if(digitalRead(PIN_BUTTON_DOWN2) == LOW)                   
{
count = 0;
while (digitalRead(PIN_BUTTON_DOWN2) == LOW)            
{
count--;
if(count < 0 && count >= -6)
{
...
matrix.beginDraw();
matrix.stroke(0xFFFFFFFF);
matrix.textScrollSpeed(20);
matrix.textFont(Font_5x7);
matrix.beginText(6, 1, 0xFFFFFF);
matrix.println(frequency);
matrix.endText(SCROLL_LEFT);
matrix.endDraw();
}

void writeFloatToEEPROM(int deviceAddress, int memoryAddress, float value) {
union {
float floatValue;
byte byteArray[4];
} 
...
if (currentMode == MODE_SHOW_TIME) {
u8g2.drawBox(0, 0, (127 * percentageOfSecondElapsed), 2);
}}
...
void timeset2() {
if (digitalRead(PIN_BUTTON_BREAK) == LOW) {
u8g2.clear();
delay(250);
clearscreen();
delay(250);
...
if (digitalRead(PIN_BUTTON_RTC) == LOW) {
RTCTime startTime(1, Month::JANUARY, 1, hours, minutes, seconds, DayOfWeek::MONDAY, SaveLight::SAVING_TIME_ACTIVE);
...
int houre = arduino.getHour();
int minute = arduino.getMinutes();
int seconds = arduino.getSeconds();
...
if (currentMillis3 - lastConnectionTime3 > postingInterval3) {
RTC_request();
}
if (houre == 0 && minute == 1 && seconds == 0) {
u8g2.clear();
}
if (houre >= 0 && houre < 8) {
Fire();
...
if (houre >= 19 && houre <= 23) {
if (currentMillis2 - lastConnectionTime2 > postingInterval2) {
http_request2();
}
Fire();
sw2();
...
float readFloatFromEEPROM(int deviceAddress, int memoryAddress) {
union {
float floatValue;
byte byteArray[4];
} 
